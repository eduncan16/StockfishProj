...
class Engine(subprocess.Popen):
    ...
    def __init__(self, stockfish_path='', depth=40, param={}):
        try:
    ...
    def setposition(self, position):
        ...
        try:
            if isinstance(position, list):
                ...
                self.isready()
            elif re.match(r'\s*^(((?:[rnbqkpRNBQKP1-8]+\/){7})[rnbqkpRNBQKP1-8]+)\s([b|w])\s([K|Q|k|q|-]{1,4})\s(-|[a-h][1-8])\s(\d+\s\d+)$', position):
                regexList = re.match(r'\s*^(((?:[rnbqkpRNBQKP1-8]+\/){7})[rnbqkpRNBQKP1-8]+)\s([b|w])\s([K|Q|k|q|-]{1,4})\s(-|[a-h][1-8])\s(\d+\s\d+)$', position).groups()
                fen = regexList[0].split(r"/")
                if len(fen) != 8:
                ...
                self.isready()
            else: raise ValueError(r"fen doesn`t match follow this example: rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1 ")  
